// Generated by Haxe 3.4.5
#ifndef INCLUDED_phoenix_Batcher
#define INCLUDED_phoenix_Batcher

#ifndef HXCPP_H
#include <hxcpp.h>
#endif

HX_DECLARE_CLASS1(luxe,Emitter)
HX_DECLARE_CLASS2(luxe,resource,Resource)
HX_DECLARE_CLASS2(luxe,structural,BalancedBST_phoenix_geometry_GeometryKey_phoenix_geometry_Geometry)
HX_DECLARE_CLASS1(phoenix,BatchState)
HX_DECLARE_CLASS1(phoenix,Batcher)
HX_DECLARE_CLASS1(phoenix,Camera)
HX_DECLARE_CLASS1(phoenix,Renderer)
HX_DECLARE_CLASS1(phoenix,Shader)
HX_DECLARE_CLASS2(phoenix,geometry,Geometry)
HX_DECLARE_CLASS2(phoenix,geometry,GeometryKey)
HX_DECLARE_CLASS3(snow,api,buffers,ArrayBufferView)

namespace phoenix{


class HXCPP_CLASS_ATTRIBUTES Batcher_obj : public hx::Object
{
	public:
		typedef hx::Object super;
		typedef Batcher_obj OBJ_;
		Batcher_obj();

	public:
		enum { _hx_ClassId = 0x2547c658 };

		void __construct( ::phoenix::Renderer _r,::String __o__name, ::Dynamic __o__max_verts);
		inline void *operator new(size_t inSize, bool inContainer=true,const char *inName="phoenix.Batcher")
			{ return hx::Object::operator new(inSize,inContainer,inName); }
		inline void *operator new(size_t inSize, int extra)
			{ return hx::Object::operator new(inSize+extra,true,"phoenix.Batcher"); }
		static hx::ObjectPtr< Batcher_obj > __new( ::phoenix::Renderer _r,::String __o__name, ::Dynamic __o__max_verts);
		static hx::ObjectPtr< Batcher_obj > __alloc(hx::Ctx *_hx_ctx, ::phoenix::Renderer _r,::String __o__name, ::Dynamic __o__max_verts);
		static void * _hx_vtable;
		static Dynamic __CreateEmpty();
		static Dynamic __Create(hx::DynamicArray inArgs);
		//~Batcher_obj();

		HX_DO_RTTI_ALL;
		hx::Val __Field(const ::String &inString, hx::PropertyAccess inCallProp);
		static bool __GetStatic(const ::String &inString, Dynamic &outValue, hx::PropertyAccess inCallProp);
		hx::Val __SetField(const ::String &inString,const hx::Val &inValue, hx::PropertyAccess inCallProp);
		static bool __SetStatic(const ::String &inString, Dynamic &ioValue, hx::PropertyAccess inCallProp);
		void __GetFields(Array< ::String> &outFields);
		static void __register();
		void __Mark(HX_MARK_PARAMS);
		void __Visit(HX_VISIT_PARAMS);
		bool _hx_isInstanceOf(int inClassId);
		::String __ToString() const { return HX_HCSTRING("Batcher","\x87","\x4a","\x61","\x8d"); }

		static void __boot();
		static int _sequence_key;
		static ::Array< ::Dynamic> all;
		static int vert_attribute;
		static int tcoord_attribute;
		static int color_attribute;
		static int normal_attribute;
		::String id;
		bool enabled;
		int layer;
		 ::luxe::structural::BalancedBST_phoenix_geometry_GeometryKey_phoenix_geometry_Geometry geometry;
		 ::luxe::Emitter emitter;
		bool tree_changed;
		 ::snow::api::buffers::ArrayBufferView pos_list;
		 ::snow::api::buffers::ArrayBufferView tcoord_list;
		 ::snow::api::buffers::ArrayBufferView color_list;
		 ::snow::api::buffers::ArrayBufferView normal_list;
		int max_verts;
		int max_floats;
		int vert_count;
		 ::phoenix::Renderer renderer;
		 ::phoenix::Camera view;
		 ::phoenix::Shader shader;
		int draw_calls;
		int dynamic_batched_count;
		int static_batched_count;
		int visible_count;
		bool log;
		::String name;
		int show_stats;
		int sequence;
		int pos_floats;
		int tcoord_floats;
		int color_floats;
		int normal_floats;
		void on(int event, ::Dynamic handler);
		::Dynamic on_dyn();

		bool off(int event, ::Dynamic handler);
		::Dynamic off_dyn();

		void add( ::phoenix::geometry::Geometry _geom,hx::Null< bool >  _force_add);
		::Dynamic add_dyn();

		void empty(hx::Null< bool >  _drop);
		::Dynamic empty_dyn();

		void destroy(hx::Null< bool >  _drop);
		::Dynamic destroy_dyn();

		void remove( ::phoenix::geometry::Geometry _geom, ::Dynamic _remove_batcher_from_geometry);
		::Dynamic remove_dyn();

		 ::phoenix::BatchState state;
		::Array< ::Dynamic> _dropped;
		void batch(hx::Null< bool >  persist_immediate);
		::Dynamic batch_dyn();

		void prune();
		::Dynamic prune_dyn();

		void draw( ::Dynamic persist_immediate);
		::Dynamic draw_dyn();

		void update_view();
		::Dynamic update_view_dyn();

		void apply_default_uniforms( ::phoenix::Shader _shader);
		::Dynamic apply_default_uniforms_dyn();

		void submit_geometry( ::phoenix::geometry::Geometry _geom, ::phoenix::Shader _shader);
		::Dynamic submit_geometry_dyn();

		void submit_buffers(int type, ::snow::api::buffers::ArrayBufferView _pos, ::snow::api::buffers::ArrayBufferView _tcoords, ::snow::api::buffers::ArrayBufferView _colors, ::snow::api::buffers::ArrayBufferView _normals);
		::Dynamic submit_buffers_dyn();

		bool submit_static_geometry( ::phoenix::geometry::Geometry geom);
		::Dynamic submit_static_geometry_dyn();

		bool submit_current_vertex_list(int type);
		::Dynamic submit_current_vertex_list_dyn();

		void geometry_batch( ::phoenix::geometry::Geometry geom);
		::Dynamic geometry_batch_dyn();

		int set_layer(int _layer);
		::Dynamic set_layer_dyn();

		virtual ::String toString();
		::Dynamic toString_dyn();

		int compare( ::phoenix::Batcher other);
		::Dynamic compare_dyn();

		::String compare_rule_to_string(int r);
		::Dynamic compare_rule_to_string_dyn();

		int compare_rule( ::phoenix::geometry::GeometryKey a, ::phoenix::geometry::GeometryKey b);
		::Dynamic compare_rule_dyn();

		int geometry_compare( ::phoenix::geometry::GeometryKey a, ::phoenix::geometry::GeometryKey b);
		::Dynamic geometry_compare_dyn();

		void list_geometry();
		::Dynamic list_geometry_dyn();

		int set_show_stats(int _value);
		::Dynamic set_show_stats_dyn();

};

} // end namespace phoenix

#endif /* INCLUDED_phoenix_Batcher */ 
